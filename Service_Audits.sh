...
service_audit() {
    ...
    # Check for critical services and ensure they are running
    ...
    # Check for unnecessary services (example: telnet, rsh)
    unnecessary_services=("telnet" "rsh" "ftp")
    for service in "${unnecessary_services[@]}"; do
        if systemctl is-active --quiet $service; then
            echo "$service is running. Stopping and disabling it..."
...
# Function to check if any service is listening on non-standard or insecure ports
check_service_ports() {
    echo -e "\nChecking for services listening on non-standard or insecure ports..."
    netstat -tuln | awk '{print $4, $7}' | grep -E ":(20|21|23|25|79|110|143|514|6000|1080)" | while read line; do
        echo "Insecure service found: $line"
    done
}
